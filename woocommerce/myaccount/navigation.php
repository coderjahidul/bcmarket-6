...
<nav class="col-xs-12 col-lg-2 lk-menu">
    <button class="button"><img src="<?php echo get_template_directory_uri(); ?>/img/menu-icons/line-menu.svg">Menu Cabinet</button>
	<ul class="lk-menu-mobile-hide">
        <li class="lk-menu-header ">
            <span>Current balance:</span>
            <span><?php echo $current_balance; ?></span>  
        </li>
		<?php  
        global $wp;
        foreach ( wc_get_account_menu_items() as $endpoint => $label ) :
            ...
            if ( $current ) {     
                $classes = 'lk-menu__active';
            }

            if(in_array(0, $chat_read) && 'tickets' === $endpoint){
                $classes = 'has_unread_message';
            }

            ?>
			<li>
				<a class="<?php echo $classes; ?>" href="<?php echo esc_url( wc_get_account_endpoint_url( $endpoint ) ); ?>">
                    <?php echo $icons[$endpoint] ?>
					<?php if($label == 'My Wallet') : ?>
						<?php echo 'Add Funds'; ?>
					<?php else : ?>
