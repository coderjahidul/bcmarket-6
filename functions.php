...
function customize_editor_menu() {
    ...
    if (current_user_can('editor')) {
        ...
        // Remove sub-menu items under "Posts"
        ...
        // Add access to necessary pages
        $allowed_pages = array(
            'users-history', // Add the slug of the "users-history" page
            'buyer-history', // Add the slug of the "buyer-history" page
            'partner-history', // Add the slug of the "partner-history" page
        );

        foreach ($allowed_pages as $page) {
            // Get the URL of the page based on the slug
            $page_url = admin_url("admin.php?page={$page}");
            // Add access to the page
            add_menu_page(__('History', 'textdomain'), __('SHistory', 'textdomain'), 'read', $page_url, '', 'dashicons-clock'); // Adjust the parameters according to your needs
        }
    }
}
add_action('admin_menu', 'customize_editor_menu');

// Prevent direct access to removed pages
function restrict_removed_pages() {
    global $pagenow;
    ...
    // Redirect if trying to access a removed page
    if (in_array($pagenow, $removed_pages)) {
        wp_redirect(admin_url());
        exit;
    }
}
add_action('admin_init', 'restrict_removed_pages');
